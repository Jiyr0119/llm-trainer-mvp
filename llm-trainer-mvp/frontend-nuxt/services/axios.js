...
// import { ElMessage } from 'element-plus' // Removed Element Plus
import { getErrorMessage, getErrorType, ERROR_TYPES } from '../utils/errorCodes';
import { API_URL } from '../config';

// Temporary mock for ElMessage until we integrate Nuxt UI Toast
const ElMessage = (options) => {
    console.log('[Toast Mock]', options);
    // specific logic can be added here or connected to a global bus
};

// 创建axios实例
const axiosInstance = axios.create({
    baseURL: API_URL || 'http://localhost:8001/api', // Updated to use imported constant
    timeout: 30000, // 30秒超时
    headers: {
...
axiosInstance.interceptors.request.use(
    config => {
        // 从本地存储获取访问令牌
        // Note: In SSR Nuxt, localStorage is only available on client. 
...
const refreshToken = async () => {
    try {
        ...
        const response = await axios.post(
            `${API_URL || 'http://localhost:8001/api'}/auth/refresh`,
...
axiosInstance.interceptors.response.use(
    ...
    async (error) => {
        ...
        // TODO: Replace with Nuxt UI compatible toast
